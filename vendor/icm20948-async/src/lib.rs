...
impl<I2C: I2c> Transport for I2cDevice<I2C> {
    ...
    async fn write_registers(&mut self, reg_addr: u8, write: &[u8]) -> Result<(), Self::Error> {
        let addr = self.address.get();
        // embassy-nrf TWIM does not merge consecutive Write ops in a transaction into one
        // frame — build a single contiguous buffer instead.
        let mut buf = [0u8; 17]; // reg addr + up to 16 data bytes
        buf[0] = reg_addr;
        let len = write.len().min(16);
        buf[1..=len].copy_from_slice(&write[..len]);
        self.bus_inner.write(addr, &buf[..=len]).await
    }
}
